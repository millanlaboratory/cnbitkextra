...
int main(int argc, char *argv[]) {
	...
	// Addition for timeout
	CcTimeValue TrialStartTic;
	float timeout;
	...
	for(idx_trial = 0; idx_trial < trials.Total(); idx_trial++) {
		...
		// Start counting trial time here
		CcTime::Tic(&TrialStartTic);
		...
		bool aboveThreeQuarterThreshold(false);
		...
		while(true) {
			...
			probs->Update(icc);
			timeout = CcTime::Toc(&TrialStartTic);
			probs->Max(&idx_hit);
			if(idx_hit == idx_class) {
				if(probs->Max(&idx_hit) < (thresholds[idx_hit] + 0.5) / 2 && 
					(!aboveThreshold || aboveThreeQuarterThreshold)) {
					CcLogInfoS("half success");	
					idMessageDev.SetEvent(p_gdf_dev.halfcorrect);
					tobiIDDev.SetMessage(&idSerializerDev);
					aboveThreshold = true;
					aboveThreeQuarterThreshold = false;
				}
				else if(probs->Max(&idx_hit) >= (thresholds[idx_hit] + 0.5) / 2 && !aboveThreeQuarterThreshold) {
					CcLogInfoS("3/4 success");	
					idMessageDev.SetEvent(p_gdf_dev.threequartercorrect);
					tobiIDDev.SetMessage(&idSerializerDev);
					aboveThreeQuarterThreshold = true;
				}
			}
			else{
				if(aboveThreeQuarterThreshold || aboveThreshold){
					CcLogInfoS("no");	
					idMessageDev.SetEvent(p_gdf_dev.belowhalfcorrect);
					tobiIDDev.SetMessage(&idSerializerDev);
			...
			if(probs->Max(&idx_hit) >= thresholds[idx_hit]) {
                ...
                tobiIDDev.SetMessage(&idSerializerDev);
                if(idx_hit == idx_class) {
                    if (t_gdf[idx_class] == 782) {
                        idMessageDev.SetEvent(FLEXION);
                        tobiIDDev.SetMessage(&idSerializerDev);
                    }
                    else if (t_gdf[idx_class] == 784) {
                        idMessageDev.SetEvent(EXTENSION);
                        tobiIDDev.SetMessage(&idSerializerDev);
                    }
                }
				...
				if ( (p_dt.timeout > 0.00f) && (timeout >= p_dt.timeout) ) {
					...
					CcLogInfoS("Timeout reached");						
					bars->ScreenSync(p_hwt.timeout);
					idm.SetEvent(p_gdf.timeout);
					
					bars->ShowBlock("Timeout");
					id.SetMessage(&ids);
					CcTime::Sleep(p_dt.boom);
					
					idm.SetEvent(p_gdf.timeout + p_gdf.off);
					id.SetMessage(&ids);
					break;
				}
			}
...
shutdown:
	...
	//SIMIS new: also signal at iD that the protocol has stopped
	// Use the same stop event as for dev
