...
typedef struct devmi_struct {
	GDFEvent correct;
	GDFEvent threequartercorrect;
	GDFEvent halfcorrect;
	GDFEvent belowhalfcorrect;
	GDFEvent right;
	GDFEvent left;
	GDFEvent start;
	GDFEvent stop;
	GDFEvent suspend;
	GDFEvent resume;
	GDFEvent device; 	// Mask for device commands
} devmi;
...
bool mi_get_devmarkers(CCfgConfig* config, devmi* dev) {
	try {
		...
		dev->correct = config->BranchEx()->QuickGDFIntEx("gdfevents/cnbi/correct");
