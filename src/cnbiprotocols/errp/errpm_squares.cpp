...
//void ErrpSquares::Setup(float sqsize) {
void ErrpSquares::Setup(void) {
	/* squares' width: to be parametrized! */
	this->_sqs = 1 / ((1+0.5f) * this->_totalsquares + 1); // size of the sq determined here
	...
	this->_color_success = dtk_green; //
	this->_color_failure = dtk_red; //
	...
	memcpy(_rgb_whitesq, _color_whitesq, 4 * sizeof(float));
	memcpy(_rgb_targetsq, _color_target, 4 * sizeof(float));
	memcpy(_rgb_cursorsq, _color_cursor, 4 * sizeof(float));
	memcpy(_rgb_success, _color_success, 4 * sizeof(float));
	memcpy(_rgb_failure, _color_failure, 4 * sizeof(float));
...
void ErrpSquares::Setup(float sqsize) {
	/* squares' width: to be parametrized! */
	this->_sqs = sqsize / ((1+0.5f) * this->_totalsquares + 1); // size of the sq determined here
	this->_stpsz = 3.0/2 * this->_sqs;
	this->_cwdth = this->_sqs*2/3;
	this->_chght = this->_sqs*2/3;
	//this->_color_fixation = dtk_aluminium_light;
	this->_color_fixation = dtk_black;
	//this->_color_whitesq = dtk_white;
	this->_color_whitesq = dtk_aluminium2_dark;
	this->_color_target = dtk_orange_dark;
	this->_color_cursor = dtk_skyblue_med;
	this->_color_success = dtk_green; //
	this->_color_failure = dtk_red; //
...
void ErrpSquares::RenderTargetSquareSuccess(float xpos, float ypos, float alpha) {
	this->_rgb_targetsq[3] = alpha;
...
void ErrpSquares::RenderTargetSquareFailure(float xpos, float ypos, float alpha) {
	this->_rgb_targetsq[3] = alpha;
...
void ErrpSquares::RenderCursorSquareSuccess(float xpos, float ypos, float alpha) {
	this->_rgb_cursorsq[3] = alpha;
...
void ErrpSquares::RenderCursorSquareFailure(float xpos, float ypos, float alpha) {
	this->_rgb_cursorsq[3] = alpha;
...
void ErrpSquares::PresentSquaresSuccess(short int targetlocation, 
		short int cursorlocation) {
	if(targetlocation == ErrpSquares::NoDraw)
		trg_alpha = 0;
	else {
	...
	this->DataWait();
	this->RenderTargetSquareSuccess(trg_xpos, trg_ypos, trg_alpha);
	this->RenderCursorSquareSuccess(cur_xpos, cur_ypos, cur_alpha);
	this->DataPost();
	
	CSmrEngineExt::RequestUpdate();
}

void ErrpSquares::PresentSquaresFailure(short int targetlocation, 
		short int cursorlocation) {
	if(targetlocation == ErrpSquares::NoDraw)
		trg_alpha = 0;
	else {
	...
	this->DataWait();
	this->RenderTargetSquareFailure(trg_xpos, trg_ypos, trg_alpha);
	this->RenderCursorSquareFailure(cur_xpos, cur_ypos, cur_alpha);
	this->DataPost();
...
void ErrpSquares::Open(float sqsize) {
	this->Setup(sqsize);	
	CSmrEngineExt::Open();
}
