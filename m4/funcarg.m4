...
# AC_SEARCH_LIBS_CLASS(CLASS, SEARCH-LIBS, [INCLUDES],
#                [ACTION-IF-FOUND], [ACTION-IF-NOT-FOUND],
#                [OTHER-LIBRARIES])
# --------------------------------------------------------
# Search for a library defining class CLASS, if it's not already available.
AC_DEFUN([AC_SEARCH_LIBS_CLASS],
[AC_LANG_PUSH([C++])
AS_VAR_PUSHDEF([ac_Search], [ac_cv_search_$1])dnl
AC_CACHE_CHECK([for library containing $1], [ac_Search],
[ac_func_search_save_LIBS=$LIBS
codeline="$1 dummy;"
AC_LANG_CONFTEST([AC_LANG_PROGRAM([$3], [[$codeline]])])
for ac_lib in '' $2; do
  if test -z "$ac_lib"; then
    ac_res="none required"
  else
